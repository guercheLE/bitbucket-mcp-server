...
## Summary
**Primary Requirement**: Inicializar projeto Bitbucket MCP Server com estrutura base, configurações constitucionais e dependências oficiais do MCP SDK.

**Technical Approach**: Implementar servidor MCP seguindo rigorosamente o protocolo oficial, com suporte multi-transporte (stdio, HTTP, SSE), detecção automática de tipo de servidor Bitbucket, registro seletivo de ferramentas baseado em capacidades, e cobertura completa de APIs Data Center (7.16+) e Cloud com TDD obrigatório (>80% cobertura).
...
**Specific Task Categories for Inicio do Projeto**:

1. **Project Structure Tasks [P]**:
   - Create src/ directory structure (server/, client/, tools/, services/, types/, utils/)
   - Create tests/ directory structure (contract/, integration/, unit/)
   - Create configuration files (tsconfig.json, jest.config.js, .eslintrc.js, .prettierrc)

2. **Dependency Management Tasks [P]**:
   - Install official MCP SDK (@modelcontextprotocol/sdk)
   - Install core dependencies (TypeScript, Zod, Axios, Winston, Commander.js)
   - Install development dependencies (Jest, ESLint, Prettier, Husky)
   - Configure package.json scripts (build, test, dev, start, cli)

3. **Configuration Tasks [P]**:
   - Configure TypeScript with strict mode
   - Configure Jest with >80% coverage requirement
   - Configure ESLint and Prettier
   - Configure environment variables (.env.example)

4. **Basic Implementation Tasks**:
   - Create basic MCP server structure (src/server/index.ts)
   - Create basic CLI client structure (src/client/cli/index.ts)
   - Implement server detection logic
   - Implement authentication hierarchy

5. **Test Structure Tasks [P]** (TDD Red Phase):
   - Create unit tests for MCP server
   - Create contract tests for project initialization API
   - Create integration tests for CLI client
   - Create tests for server detection
   - Create tests for authentication

6. **Quality Gates Tasks**:
   - Configure Git hooks (Husky, lint-staged)
   - Set up pre-commit quality checks
   - Configure test coverage reporting
   - Set up Constitution compliance validation

**Ordering Strategy**:
- TDD order: Tests before implementation (Article V compliance)
- Dependency order: Structure → Dependencies → Configuration → Implementation → Tests
- Mark [P] for parallel execution (independent files)
- Constitution compliance validation at each step

**Estimated Output**: 20-25 numbered, ordered tasks in tasks.md

**Constitution Compliance**:
- All tasks must follow Article V (Test-First) - tests written before implementation
- All tasks must follow Article I (MCP Protocol First) - official SDK usage
- All tasks must follow Article VII (Simplicity) - no unnecessary complexity
- All tasks must include Constitution compliance validation

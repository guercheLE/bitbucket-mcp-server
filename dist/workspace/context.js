 ...
 */
import { WorkspaceErrorType, WorkspaceStatus, } from './types.js';
/**
 * Workspace Context Provider
 *
...
export class WorkspaceContextProvider {
    ...
    requestHistory = [];
    constructor(workspaceManager, config = {}) {
     ...
     */
    async resolveContext(workspaceIdOrSlug, requestId, toolName, parameters) {
        ...
        try {
            ...
            if (!targetWorkspaceId) {
                throw this.createContextError(WorkspaceErrorType.WORKSPACE_NOT_FOUND, 'No workspace specified and no default workspace configured');
            }
            // Resolve workspace
            ...
            if (workspace.status === WorkspaceStatus.INACTIVE || workspace.status === WorkspaceStatus.DISCONNECTED) {
                throw this.createContextError(WorkspaceErrorType.WORKSPACE_UNAVAILABLE, `Workspace '${workspace.name}' is not available (status: ${workspace.status})`, workspace.id);
            }
            if (workspace.status === WorkspaceStatus.ERROR) {
                throw this.createContextError(WorkspaceErrorType.WORKSPACE_UNAVAILABLE, `Workspace '${workspace.name}' is in error state`, workspace.id);
            }
     ...
     */
    async resolveCrossWorkspaceContext(workspaceIdsOrSlugs, aggregateResults = true, requestId, toolName, parameters) {
        const timestamp = new Date().toISOString();
        const resolvedRequestId = requestId || this.generateRequestId();
        try {
            // Check if cross-workspace operations are allowed
            if (!this.config.allowCrossWorkspace) {
                throw this.createContextError(WorkspaceErrorType.PERMISSION_DENIED, 'Cross-workspace operations are not allowed');
            }
            // Validate target count
            if (workspaceIdsOrSlugs.length > this.config.maxCrossWorkspaceTargets) {
                throw this.createContextError(WorkspaceErrorType.INVALID_CONFIGURATION, `Too many workspace targets. Maximum allowed: ${this.config.maxCrossWorkspaceTargets}`);
            }
            if (workspaceIdsOrSlugs.length === 0) {
                throw this.createContextError(WorkspaceErrorType.INVALID_CONFIGURATION, 'No workspace targets specified for cross-workspace operation');
            }
                ...
                try {
                    ...
                    if (workspace.status === WorkspaceStatus.ACTIVE || workspace.status === WorkspaceStatus.CONFIGURING) {
                        ...
                        if (workspace.features.crossWorkspaceOperations) {
                            ...
                            errors.push(this.createContextError(WorkspaceErrorType.PERMISSION_DENIED, `Workspace '${workspace.name}' does not support cross-workspace operations`, workspace.id));
                        }
                    }
                    else {
                        errors.push(this.createContextError(WorkspaceErrorType.WORKSPACE_UNAVAILABLE, `Workspace '${workspace.name}' is not available (status: ${workspace.status})`, workspace.id));
                    }
                }
                catch (error) {
            ...
            if (resolvedWorkspaces.length === 0) {
                throw this.createContextError(WorkspaceErrorType.WORKSPACE_UNAVAILABLE, 'No valid workspaces available for cross-workspace operation', undefined, { errors });
            }
     ...
     */
    async createWorkspaceContext(workspace, requestId) {
     ...
     */
    async resolveWorkspace(workspaceIdOrSlug) {
        ...
        if (!workspace) {
            throw this.createContextError(WorkspaceErrorType.WORKSPACE_NOT_FOUND, `Workspace not found: ${workspaceIdOrSlug}`);
        }
        return workspace;
    }
    /**
     ...
     */
    getWorkspaceRequests(workspaceId, limit = 100) {
     ...
     */
    validateCrossWorkspaceContext(context) {
        const errors = [];
     ...
     */
    createContextError(type, message, workspaceId, details) {
        const error = {
    ...
    handleError(error, context) {
        ...
        if (error instanceof Error) {
            return this.createContextError(WorkspaceErrorType.INVALID_CONFIGURATION, `${context}: ${error.message}`, undefined, { originalError: error });
        }
        return this.createContextError(WorkspaceErrorType.INVALID_CONFIGURATION, `${context}: Unknown error`, undefined, { originalError: error });
    }
    // ============================================================================

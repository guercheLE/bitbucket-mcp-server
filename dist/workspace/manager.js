...
import { DEFAULT_HEALTH_CHECK_INTERVAL, isWorkspaceConfig, MAX_WORKSPACES, WORKSPACE_ID_PATTERN, WorkspaceConfigSchema, WorkspaceErrorType, WorkspacePriority, WorkspaceQuerySchema, WorkspaceRegistrationSchema, WorkspaceStatus, WorkspaceUpdateSchema, } from './types.js';
/**
 * Workspace Manager Events
 */
export var WorkspaceManagerEvents;
(function (WorkspaceManagerEvents) {
    WorkspaceManagerEvents["WORKSPACE_ADDED"] = "workspace_added";
    WorkspaceManagerEvents["WORKSPACE_UPDATED"] = "workspace_updated";
    WorkspaceManagerEvents["WORKSPACE_REMOVED"] = "workspace_removed";
    WorkspaceManagerEvents["WORKSPACE_STATUS_CHANGED"] = "workspace_status_changed";
    WorkspaceManagerEvents["HEALTH_CHECK_COMPLETED"] = "health_check_completed";
    WorkspaceManagerEvents["ERROR_OCCURRED"] = "error_occurred";
})(WorkspaceManagerEvents || (WorkspaceManagerEvents = {}));
/**
 * Workspace Manager Service
 *
...
export class WorkspaceManager extends EventEmitter {
    ...
    async shutdown() {
        try {
            ...
            for (const [id, workspace] of Array.from(this.workspaces)) {
    ...
    async registerWorkspace(params) {
        ...
        try {
            ...
            if (this.workspaces.size >= this.config.maxWorkspaces) {
                throw this.createWorkspaceError(WorkspaceErrorType.INVALID_CONFIGURATION, `Maximum number of workspaces (${this.config.maxWorkspaces}) exceeded`, undefined, { currentCount: this.workspaces.size });
            }
            // Generate workspace ID from slug
            const workspaceId = this.generateWorkspaceId(validatedParams.slug);
            // Check for existing workspace
            if (this.workspaces.has(workspaceId)) {
                throw this.createWorkspaceError(WorkspaceErrorType.WORKSPACE_ALREADY_EXISTS, `Workspace with ID '${workspaceId}' already exists`, workspaceId);
            }
            // Check for duplicate slug
            if (this.findWorkspaceBySlug(validatedParams.slug)) {
                throw this.createWorkspaceError(WorkspaceErrorType.WORKSPACE_ALREADY_EXISTS, `Workspace with slug '${validatedParams.slug}' already exists`, workspaceId);
            }
    ...
    async updateWorkspace(workspaceId, updates) {
        ...
        try {
            ...
            if (!existingWorkspace) {
                throw this.createWorkspaceError(WorkspaceErrorType.WORKSPACE_NOT_FOUND, `Workspace with ID '${workspaceId}' not found`, workspaceId);
            }
    ...
    queryWorkspaces(params = {}) {
        try {
            ...
            if (validatedParams.tags && validatedParams.tags.length > 0) {
                workspaces = workspaces.filter(w => validatedParams.tags.some(tag => w.metadata.tags.includes(tag)));
            }
    ...
    async performHealthChecks() {
        ...
        for (const [workspaceId, workspace] of Array.from(this.workspaces)) {
     ...
     */
    async performWorkspaceHealthCheck(workspaceId, timestamp) {
     ...
     */
    async checkWorkspaceHealth(workspace) {
     ...
     */
    createWorkspaceContext(workspaceId, requestId) {
        const workspace = this.workspaces.get(workspaceId);
        if (!workspace) {
            return null;
        }
     ...
     */
    createWorkspaceError(type, message, workspaceId, details) {
        const error = {
            name: 'WorkspaceError',
            message,
            type,
     ...
     */
    handleError(error, workspaceId, context) {
        if (error instanceof Error && 'type' in error) {
            return error;
        }
        if (error instanceof Error) {
            return this.createWorkspaceError(WorkspaceErrorType.INVALID_CONFIGURATION, context ? `${context}: ${error.message}` : error.message, workspaceId, { originalError: error });
        }
        return this.createWorkspaceError(WorkspaceErrorType.INVALID_CONFIGURATION, context ? `${context}: Unknown error` : 'Unknown error', workspaceId, { originalError: error });
    }
    // ============================================================================
    // Public Properties
    // ============================================================================

...
export class ToolRegistry extends EventEmitter {
    ...
    searchTools(query) {
        ...
        return Array.from(this.tools.values())
            .filter(tool => tool.enabled && (tool.name.toLowerCase().includes(lowerQuery) ||
     ...
     */
    validateToolAuthentication(tool, context) {
        ...
        if (tool.authentication?.permissions && tool.authentication.permissions.length > 0) {
            ...
            const hasRequiredPermissions = tool.authentication.permissions.every((permission) => userPermissions.includes(permission));
        ...
        if (tool.authentication?.scopes && tool.authentication.scopes.length > 0) {
            ...
            const hasRequiredScopes = tool.authentication.scopes.every((scope) => userScopes.includes(scope));
     ...
     */
    addToCategory(toolName, category) {
     ...
     */
    removeFromCategory(toolName, category) {
     ...
     */
    updateExecutionStats(toolName, startTime, success) {
        const stats = this.executionStats.get(toolName);
        if (!stats) {
            return;
        }

 ...
 */
import { AuditEventType } from './auth-audit-logger';
/**
 * Security Requirements Validator
 *
...
export class SecurityRequirementsValidator {
    ...
    oauthManager;
    config;
    constructor(cryptoService, auditLogger, rateLimiter, securityHeaders, tokenStorage, sessionManager, oauthManager, config) {
     ...
     */
    async validateCryptographicSecurity() {
        const results = [];
        try {
            ...
            const decryptedToken = await this.cryptoService.decryptToken(encryptedToken);
            results.push({
    ...
    async validateSessionSecurity() {
        ...
        try {
            ...
            const sessionResponse = await this.sessionManager.createSession('test-session', testApplication, accessToken, refreshToken, userInfo);
            results.push({
    ...
    async validateAuditLogging() {
        ...
        try {
            ...
            await this.auditLogger.logEvent(AuditEventType.AUTH_LOGIN_SUCCESS, 'Security validation test', {
                userId: 'user123',
                clientId: 'client456',
     ...
     */
    async validatePerformanceSecurity() {
        const results = [];
        try {
     ...
     */
    meetsMinimumSecurityRequirements(report) {
        return report.criticalFailures === 0 && report.overallStatus !== 'NON_COMPLIANT';
    }
}
//# sourceMappingURL=security-requirements-validator.js.map
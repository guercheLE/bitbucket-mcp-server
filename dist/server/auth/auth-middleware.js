...
export class AuthenticationMiddleware {
    authManager;
    requireAuth;
    constructor(authManager, requireAuth = true) {
     ...
     */
    async authenticateRequest(request, clientSession, server) {
        try {
            ...
            const authResponse = await this.authManager.authenticateRequest(authInfo?.authorization, authInfo?.sessionId);
     ...
     */
    async createToolExecutionContext(toolName, params, clientSession, server, userSession) {
     ...
     */
    async validateToolPermissions(toolName, userSession, requiredPermissions = []) {
     ...
     */
    handleAuthenticationError(error, request) {
        // Log security events

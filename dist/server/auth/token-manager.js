...
export class TokenManager extends EventEmitter {
     ...
     */
    async refreshAccessToken(refreshTokenId, applicationId, baseUrl, instanceType) {
        try {
            // Validate refresh token
            ...
            const tokenResponse = await apiClient.refreshAccessToken(applicationId, refreshToken.token);
     ...
     * Revoke access token
     */
    async revokeAccessToken(tokenId, applicationId, baseUrl, instanceType) {
        try {
     ...
     */
    async revokeRefreshToken(tokenId, applicationId, baseUrl, instanceType) {
        try {
            // Get token before revoking
            const tokenResponse = await this.getRefreshToken(tokenId);
            if (!tokenResponse.success || !tokenResponse.data) {
     ...
     */
    async revokeUserTokens(userId, applicationId, baseUrl, instanceType) {
        try {

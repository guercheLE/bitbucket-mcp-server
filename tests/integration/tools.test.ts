...
const GetOpenPullRequestsOutput = z.array(PullRequestSchema.pick({ id: true, title: true, state: true }));
...
registry.register({
    ...
    handler: async ({ projectKey, repositorySlug }: { projectKey: string; repositorySlug: string }) => {
        return [
            {
                id: 101,
...
describe("call-id", () => {
    it("executes the operation handler and validates the response", async () => {
        ...
        const result = (await execution.execute("get-open-pull-requests", {
            projectKey: "PROJ",
            repositorySlug: "repo"
        })) as Array<{ id: number; title: string; state: string }>;

...
describeTransportIntegration("Transports", () => {
    ...
    it("streams SSE events for subscribed topics", async () => {
        ...
        try {
            await new Promise<void>((resolve, reject) => {
                const timeout = setTimeout(() => reject(new Error("Timed out waiting for SSE events")), 5_000);
    ...
    it("delivers HTTP streaming chunks for resource updates", async () => {
        ...
        try {
            await new Promise<void>((resolve, reject) => {
                const timeout = setTimeout(() => reject(new Error("Timed out waiting for streaming chunks")), 5_000);

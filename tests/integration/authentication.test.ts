...
interface AuthServiceStub {
  authenticateWithFallback: jest.Mock<
    Promise<AuthFallbackResult>,
    [AuthCredentials, AuthMethod[]?]
  >;
...
const createAuthServiceStub = (overrides?: Partial<AuthServiceStub>): AuthServiceStub => {
  ...
  const stub: AuthServiceStub = {
    authenticateWithFallback: jest.fn().mockResolvedValue({
      authenticated: true,
      user: { id: 'user-1', name: 'Integration' },
      methodUsed: 'oauth2',
    }),
    ...
    hasStrategy: jest
      .fn()
      .mockImplementation((method: AuthMethod) => defaultPriority.includes(method)),
...
describeAuthIntegration('authentication integration', () => {
  ...
  it('applies configured priority order when authenticating', async () => {
    ...
    const authService = createAuthServiceStub({
      getPriority: jest.fn().mockReturnValue([...customPriority]),
    });
  ...
  it('returns 401 when authentication fails despite credentials', async () => {
    const authService = createAuthServiceStub();
    authService.authenticateWithFallback.mockResolvedValue({
      authenticated: false,
      methodUsed: null,
    });

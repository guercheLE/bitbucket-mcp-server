...
const startServer = async (options: StartServerOptions = {}) => {
    const bitbucketService = options.bitbucketService ?? new SuccessfulBitbucketServiceStub({ version: "8.0.0", type: "Server" });
...
describe("security integration", () => {
    ...
    it("reports degraded health when Bitbucket connectivity fails", async () => {
        ...
        const { server, client, bitbucketService } = await startServer({ bitbucketService: failingService });

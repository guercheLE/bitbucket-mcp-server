...
describe('console client semantic workflow', () => {
  ...
  it('supports the search → get → call workflow', async () => {
    ...
    await run(
      ['node', 'mcp-client', 'search-ids', '--query', 'projects'],
      {},
      {
    ...
    await run(
      ['node', 'mcp-client', 'get-id', '--endpoint-id', 'GET /rest/api/1.0/projects'],
      {},
      {
        createMcpService: factory,
        stdout: stdout.writable,
    ...
    await run(
      ['node', 'mcp-client', 'call-id', '--endpoint-id', 'GET /rest/api/1.0/projects'],
      {},
      {
        createMcpService: factory,
        stdout: stdout.writable,

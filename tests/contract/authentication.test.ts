...
describe('authentication contracts', () => {
  ...
  const buildService = (
    ...
    priority: AuthMethod[] = ['oauth2', 'bearer', 'apiKey', 'basic'],
  ) => {
    ...
    it('uses the highest priority strategy that succeeds', async () => {
      const strategies: Partial<Record<AuthMethod, AuthStrategy>> = {
        ...
        apiKey: {
          ...
          authenticate: jest
            .fn()
            .mockResolvedValue({ authenticated: true, user: { id: '42', name: 'API' } }),
        },
      };

      const service = buildService(strategies);

      const result = await service.authenticateWithFallback(buildCredentials(), [
        'oauth2',
        'bearer',
        'apiKey',
      ]);
      ...
      expect((strategies.apiKey as AuthStrategy).authenticate).toHaveBeenCalledWith({
        key: 'api-key',
      });
    });

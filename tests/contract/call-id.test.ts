...
describe('call-id tool contract', () => {
  ...
  const _buildHttpClient = <T = unknown>(response: AxiosResponse<T> | Promise<AxiosResponse<T>>) =>
    jest
      .fn()
      .mockImplementation((_config: AxiosRequestConfig) =>
        Promise.resolve(response),
      ) as jest.MockedFunction<(config: AxiosRequestConfig) => Promise<AxiosResponse<T>>>;

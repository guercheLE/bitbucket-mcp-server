...
const OPERATION_ID = "bitbucket.pull-requests.approve";

describe("bitbucket.pull-requests.approve contract", () => {
    ...
    it("describes the REST operation metadata", async () => {
        ...
        expect(operation.description.toLowerCase()).toContain("approve");
    });
    ...
    it("requires workspace, repo_slug and pull_request_id", async () => {
        ...
        if (!result.success) {
            ...
            expect(missingPaths).toEqual(expect.arrayContaining(["workspace", "repo_slug", "pull_request_id"]));
        }
    });

    it("accepts a minimal approval payload", async () => {
        const service = buildService();
        const operation = await service.getOperation(OPERATION_ID);

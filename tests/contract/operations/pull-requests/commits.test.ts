...
const OPERATION_ID = "bitbucket.pull-requests.commits";

describe("bitbucket.pull-requests.commits contract", () => {
    ...
    it("describes the REST operation metadata", async () => {
        ...
        expect(operation.description.toLowerCase()).toContain("commit");
    });
    ...
    it("accepts minimal commit list parameters", async () => {
        const service = buildService();
        const operation = await service.getOperation(OPERATION_ID);
    ...
    it("rejects pagelen higher than Bitbucket maximum", async () => {
        const service = buildService();
        const operation = await service.getOperation(OPERATION_ID);

...
describe("bitbucket.pull-requests.merge contract", () => {
    ...
    it("describes the REST operation metadata", async () => {
        ...
        expect(operation.path).toBe("/2.0/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/merge");
    ...
    it("requires workspace, repo_slug, pull_request_id and body", async () => {
        ...
        if (!result.success) {
            ...
            expect(missingPaths).toEqual(expect.arrayContaining(["workspace", "repo_slug", "pull_request_id", "body"]));
        }
    });
    ...
    it("rejects unknown merge strategies", async () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
            const issue = result.error.issues.find((candidate) => candidate.path.join(".") === "body.merge_strategy");

...
const OPERATION_ID = 'bitbucket.pull-requests.activities';

describe('bitbucket.pull-requests.activities contract', () => {
  ...
  it('describes the REST operation metadata', async () => {
    ...
    expect(operation.path).toBe(
      '/2.0/repositories/{workspace}/{repo_slug}/pullrequests/{pull_request_id}/activities',
    );
    expect(operation.description.toLowerCase()).toContain('activity');
  });
  ...
  it('accepts minimal activity list parameters', async () => {
    const service = buildService();
    const operation = await service.getOperation(OPERATION_ID);
  ...
  it('rejects pagelen values outside Bitbucket bounds', async () => {
    const service = buildService();
    const operation = await service.getOperation(OPERATION_ID);

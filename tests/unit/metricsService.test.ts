...
describe('MetricsService', () => {
  ...
  it('records HTTP requests with default configuration', async () => {
    ...
    expect(histogramMock.observe).toHaveBeenCalledWith(
      { method: 'GET', route: '/health', status: '200' },
      0.125,
    );
    expect(Gauge).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'health_check_success_rate' }),
    );
  ...
  it('honors custom prefix and disables default metrics when requested', () => {
    ...
    expect(Counter).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'test_http_requests_total' }),
    );
    expect(Histogram).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'test_http_request_duration_seconds' }),
    );

...
const createStrategy = <TInput = AuthCredentials>(
    name: AuthMethod,
    implementation: {
        authenticate?: jest.Mock<Promise<{ authenticated: boolean; user?: { id: string; name: string } }>, [TInput]>;
        canHandle?: jest.Mock<boolean, [AuthCredentials]>;
    } = {}
): AuthStrategy<TInput> & { authenticate: jest.Mock; canHandle?: jest.Mock } => {
    const authenticate = implementation.authenticate ?? jest.fn().mockResolvedValue({ authenticated: true });
...
describe("AuthService", () => {
    ...
    it("tries fallback strategies according to priority and credentials", async () => {
        ...
        const bearerStrategy = createStrategy("bearer", {
            authenticate: jest.fn().mockResolvedValue({ authenticated: true, user: { id: "b-1", name: "Bearer User" } })
        });
        ...
        expect(result).toEqual({ authenticated: true, user: { id: "b-1", name: "Bearer User" }, methodUsed: "bearer" });
    });
    ...
    it("skips strategies when canHandle returns false or credentials missing", async () => {
        ...
        const basicStrategy = createStrategy("basic", {
            authenticate: jest.fn().mockResolvedValue({ authenticated: true, user: { id: "ok", name: "Basic" } })
        });
        ...
        expect(result).toEqual({ authenticated: true, user: { id: "ok", name: "Basic" }, methodUsed: "basic" });
    });
    ...
    it("returns unauthenticated result when all strategies fail", async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith("Authentication strategy failed", { method: "oauth2", error: "nope" });

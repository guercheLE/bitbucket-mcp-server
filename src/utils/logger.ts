import { format, transports, createLogger as winstonCreateLogger, type Logger, type transport } from "winston";
...
const sanitizeInfo = format((info) => {
    const additionalKeys = Object.keys(info).filter((key) => !["level", "message", "timestamp", "service"].includes(key));
...
export const createLogger = (options: LoggerOptions = {}): Logger => {
    const loggerTransports = options.transports && options.transports.length > 0 ? options.transports : [new transports.Console()];

    return winstonCreateLogger({
        level: resolveLevel(options.level),
        defaultMeta: { service: "bitbucket-mcp-server", ...options.defaultMeta },
        format: format.combine(format.timestamp(), format.errors({ stack: true }), sanitizeInfo(), format.json()),

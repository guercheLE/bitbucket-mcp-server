...
export class PermissionManager extends EventEmitter {
    ...
    private decisionCache: Map<string, { decision: PermissionDecision; expiresAt: number }> = new Map();
    ...
    async createPermissionRule(rule: Omit<PermissionRule, 'id'>): Promise<PermissionRule> {
    ...
    async updatePermissionRule(ruleId: string, updates: Partial<PermissionRule>): Promise<PermissionRule> {
    ...
    async evaluatePermission(context: PermissionContext): Promise<PermissionDecision> {
        // Validate context
        ...
        const appliedRules: PermissionDecision['appliedRules'] = [];
    ...
    private findApplicableRules(context: PermissionContext): PermissionRule[] {
        ...
        for (const rule of Array.from(this.permissionRules.values())) {
            ...
            if (!resourceTypeMatch && context.resourceId) continue;
            ...
            if (!roleMatch && rule.roles.length > 0 && !rule.roles.includes('*')) continue;
    ...
    private async evaluateRuleConditions(rule: PermissionRule, context: PermissionContext): Promise<boolean> {
    ...
    private evaluateTimeConstraints(constraints: NonNullable<PermissionRule['timeConstraints']>, context: PermissionContext): boolean {
    ...
    private evaluateCondition(attributeValue: any, operator: string, expectedValue: any): boolean {
        switch (operator) {
            case 'eq':
    ...
    private generateActionId(name: string, category: string): string {
        return `${category}:${name.toLowerCase().replace(/[^a-z0-9_-]/g, '_')}`;
    }

    private generateRuleId(name: string, effect: string): string {
     ...
     * Initialize default resources and actions
     */
    private async initializeDefaultResourcesAndActions(): Promise<void> {
        ...
        // Default actions
        const defaultActions: Omit<Action, 'id'>[] = [
            // Read actions
     ...
     */
    getPermissionStats(): {
        resources: { total: number; byType: Record<string, number>; active: number };
        actions: { total: number; byCategory: Record<string, number>; byRiskLevel: Record<string, number> };
        rules: { total: number; active: number; byEffect: Record<string, number> };

...
import {
  ...
  type HttpStreamTransportOptions,
} from './transports/httpStream';
  ...
  const parsed = ServerConfigSchema.safeParse({
    port:
      typeof portCandidate === 'string'
        ...
        : (portCandidate ?? DEFAULT_HTTP_PORT),
...
const resolveCredentials = (
  overrides: Partial<BitbucketCredentials> | undefined,
  env: NodeJS.ProcessEnv,
...
const defaultHttpServerFactory: HttpServerFactory = ({
  ...
  const buildServer = () => {
    ...
    if (authService && authService.hasStrategies()) {
      const authLogger =
        typeof logger.child === 'function' ? logger.child({ scope: 'auth-middleware' }) : logger;
    ...
    app.post('/shutdown', async (_req, res) => {
      ...
      try {
        ...
        logger.error('Failed to stop server from shutdown endpoint', {
    ...
    app.get('/transports/http-stream', async (req, res, next) => {
      try {
        const resourceId =
          typeof req.query.resourceId === 'string' ? req.query.resourceId : 'default';
        ...
        await httpStreamTransport.handle(req, res, async (stream) => {
          ...
          await stream.write({
            data: JSON.stringify({ status: 'ready', resourceId }) + '\n',
            isLast: false,
          });
...
export const createServer = (options: ServerOptions = {}): ServerInstance => {
  ...
  const loggerFactory =
    options.dependencies?.createLogger ??
  ...
  const bitbucketServiceFactory =
    ...
    ((creds: BitbucketCredentials, svcLogger: Logger) =>
      new BitbucketService(creds, { logger: svcLogger }));
  const bitbucketService =
    options.dependencies?.bitbucketService ??
    bitbucketServiceFactory(credentials, logger.child({ scope: 'bitbucket' }));

  const mcpServerFactory =
    options.dependencies?.createMcpServer ??
    ((_logger: Logger) =>
      new McpServer(
        {
  ...
  const stdioTransport =
    options.dependencies?.createStdioTransport?.() ?? new StdioServerTransport();
  const httpTransport =
    options.dependencies?.createHttpTransport?.() ??
  ...
  const rateLimiterFactory =
    options.dependencies?.createRateLimiter ??
    ((rateLimiterOptions?: CombinedRateLimiterOptions) => createRateLimiter(rateLimiterOptions));
  const rateLimiter =
  ...
  const metricsServiceFactory =
    options.dependencies?.createMetricsService ??
    ((metricsOptions?: MetricsServiceOptions) => createMetricsService(metricsOptions));
  ...
  const i18nServiceFactory =
    options.dependencies?.createI18nService ??
    ((i18nOptions: Parameters<typeof createI18nService>[0]) => createI18nService(i18nOptions));
  const i18nService =
    options.dependencies?.i18nService ??
    i18nServiceFactory({
      fallbackLng: 'en',
      supportedLngs: ['en'],
  ...
  const sseTransportFactory =
    options.dependencies?.createSseTransport ??
    ((sseOptions?: SseTransportOptions) => createSseTransport(sseOptions));
  const sseTransport = options.dependencies?.sseTransport ?? sseTransportFactory();

  const httpStreamTransportFactory =
    options.dependencies?.createHttpStreamTransport ??
    ((streamOptions?: HttpStreamTransportOptions) => createHttpStreamTransport(streamOptions));
  const httpStreamTransport =
    options.dependencies?.httpStreamTransport ?? httpStreamTransportFactory();

  const eventBus = options.dependencies?.eventBus ?? new EventEmitter();

  const vectorDbService =
    options.dependencies?.vectorDbService ??
    new VectorDBService({
      logger: logger.child({ scope: 'vector-db-service' }),
    });

  const schemaService =
    options.dependencies?.schemaService ??
    new SchemaService({
      logger: logger.child({ scope: 'schema-service' }),
    });

  const authServiceFactory =
    options.dependencies?.createAuthService ??
    ((config: AppConfig['authentication'], svcLogger: Logger) =>
      new AuthService({
        config,
        logger: svcLogger,
      }));
  const authService =
    options.dependencies?.authService ??
    authServiceFactory(appConfig.authentication, logger.child({ scope: 'auth-service' }));
  ...
  const buildToolResult = (payload: unknown): CallToolResult => {
    ...
    async start(): Promise<void> {
      ...
      startPromise = (async () => {
        ...
        try {
          ...
          const normalized =
            error instanceof BitbucketServiceError
              ? error
              : new BitbucketServiceError((error as Error).message, error);

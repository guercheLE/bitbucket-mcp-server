...
export class MFAManager {
     ...
     */
    async generateSecret(userLabel: string, issuer?: string): Promise<MFASetupResult> {
        try {
            // Generate secret
     ...
     */
    private getTokenTime(token: string, secret: string): Date | null {
        try {
     ...
     */
    validateConfiguration(config: MFAConfiguration): { valid: boolean; errors: string[] } {
        const errors: string[] = [];

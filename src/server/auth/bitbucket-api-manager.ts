...
export class BitbucketAPIManager extends EventEmitter {
  private instances: Map<string, BitbucketAuthenticatedClient> = new Map();
  private instanceConfigs: Map<string, BitbucketInstanceConfig> = new Map();
  private healthStatus: Map<string, InstanceHealthStatus> = new Map();
  ...
  getEnabledInstances(): Map<string, BitbucketAuthenticatedClient> {
    const enabled = new Map<string, BitbucketAuthenticatedClient>();
  ...
  ): Promise<{ instanceId: string; response: any }> {
    try {
   ...
   */
  private selectBestInstance(preferredInstanceId?: string): string {
   ...
   */
  private stopHealthMonitoring(instanceId: string): void {
  ...
  private async performHealthCheck(instanceId: string): Promise<void> {
    ...
    try {
      ...
      const healthStatus: InstanceHealthStatus = {
        instanceId,
        isHealthy,
        lastCheck: new Date(),
        responseTime,

...
const isPublicPath = (pathname: string, patterns: (string | RegExp)[]): boolean => {
...
const extractCredentials = (req: Request): AuthCredentials => {
  ...
  const accessTokenHeader =
    typeof req.headers['x-access-token'] === 'string' ? req.headers['x-access-token'].trim() : null;
  if (accessTokenHeader) {
    credentials.oauth2 = { accessToken: accessTokenHeader };
  }

  const bearerHeader =
    typeof req.headers['x-auth-token'] === 'string' ? req.headers['x-auth-token'].trim() : null;
  if (bearerHeader) {
    ...
    try {
      ...
      if (!result.authenticated) {
        logger?.warn?.('Authentication failed', {
          path: pathname,
          strategies: authService.getRegisteredStrategies(),
        });

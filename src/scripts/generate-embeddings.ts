...
const createDeterministicEmbedder = (
  dimensions: number = DETERMINISTIC_EMBEDDING_DIMENSIONS,
): Embedder => {
...
const buildFallbackRecord = (operation: OperationContract): ApiOperationSource => {
  ...
  const cleaned =
    normalizedDescription.length > 0
      ? stripTrailingPeriod(normalizedDescription)
      : toTitleCase(operation.id);
...
const resolveOperationCandidates = async (
  ...
  try {
    ...
    logger.warn(
      `Unable to load metadata file at ${sourcePath}: ${error instanceof Error ? error.message : error}`,
    );
  }
...
export const generateEmbeddings = async (
  options: GenerateEmbeddingsOptions = {},
): Promise<GenerateEmbeddingsSummary> => {
  ...
  if (options.embedder) {
    ...
    const dimensionsHint =
      options.embeddingDimensions && options.embeddingDimensions > 0
  ...
  if (summary.total === 0) {
    ...
    if (usingDeterministicEmbedder) {
      logger.info(
        `Generating embeddings for ${summary.total} operations using deterministic local vectors.`,
      );
    } else {

...
const pathParameter = z
  .string({ description: 'Restrict the diff to a specific file path.' })
  .min(1);
const ignoreWhitespaceParameter = z.boolean({
  description: 'Whether to ignore whitespace-only changes.',
});

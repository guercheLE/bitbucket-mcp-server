import { SearchIdsParams, type SearchIdsParamsInput, SearchIdsResponse, type SearchIdsResponseOutput } from "../contracts/search-ids";
...
export interface VectorDbClient {
    search(query: string, options?: { limit?: number }): Promise<Array<{ id: string; description: string; score: number }>>;
}
/**
...
export const createSearchIdsTool = (
    ...
    const logger = dependencies.logger ?? createLogger({ level: "info", defaultMeta: { scope: "tool:search-ids" } });
    ...
    const handler = async (params: SearchIdsParamsInput): Promise<SearchIdsResponseOutput> => {
        ...
        logger.debug?.("Vector search completed", { query: parsed.query, resultCount: results.length, limit });

...
// Reset Project Avatar Schema
const ResetProjectAvatarSchema = z.object({
...
export const resetProjectAvatarTool: MCPTool = {
  name: 'mcp_bitbucket_project_reset_avatar',
  description: 'Reset project avatar to default in Bitbucket Data Center',
  inputSchema: ResetProjectAvatarSchema.shape,
  handler: async (params): Promise<ToolResult> => {
    const startTime = Date.now();
    const logger = loggerService.getLogger('mcp-tool', {
      tool: 'resetProjectAvatar',
      serverType: ['datacenter'],
      operation: 'reset_project_avatar',
      category: 'unknown',
    });

    try {
      logger.info('Resetting project avatar', {
        params: {
      ...
      const validatedParams = ResetProjectAvatarSchema.parse(params);
      const config = buildConfig(validatedParams);

      const result = await bitbucketAPIService.post(
        config,
        `/rest/api/1.0/projects/${validatedParams.projectKey}/avatar/reset`
      );
      ...
      if (result.success) {
        ...
        logger.info('Project avatar reset successfully', {
        ...
        logger.warn('Failed to reset project avatar', {
      ...
      logger.error('Reset project avatar tool error', {

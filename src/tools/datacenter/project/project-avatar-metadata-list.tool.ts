...
// List Project Avatar Metadata Schema
const ListProjectAvatarMetadataSchema = z.object({
...
export const listProjectAvatarMetadataTool: MCPTool = {
  name: 'mcp_bitbucket_project_list_avatar_metadata',
  description: 'List project avatar metadata from Bitbucket Data Center',
  inputSchema: ListProjectAvatarMetadataSchema.shape,
  handler: async (params): Promise<ToolResult> => {
    const startTime = Date.now();
    const logger = loggerService.getLogger('mcp-tool', {
      tool: 'listProjectAvatarMetadata',
      serverType: ['datacenter'],
      operation: 'list_project_avatar_metadata',
      category: 'unknown',
    });

    try {
      logger.info('Listing project avatar metadata', {
        params: {
      ...
      const validatedParams = ListProjectAvatarMetadataSchema.parse(params);
      ...
      if (result.success) {
        const metadata = assertPaginatedDataResponse(result.data).data.values;

        logger.info('Project avatar metadata listed successfully', {
        ...
        logger.warn('Failed to list project avatar metadata', {
      ...
      logger.error('List project avatar metadata tool error', {
  ...
  category: 'project',
  operation: 'list_avatar_metadata',
};

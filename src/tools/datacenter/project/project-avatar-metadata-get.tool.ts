...
// Get Project Avatar Metadata Schema
const GetProjectAvatarMetadataSchema = z.object({
...
export const getProjectAvatarMetadataTool: MCPTool = {
  name: 'mcp_bitbucket_project_get_avatar_metadata',
  description: 'Get project avatar metadata from Bitbucket Data Center',
  inputSchema: GetProjectAvatarMetadataSchema.shape,
  handler: async (params): Promise<ToolResult> => {
    const startTime = Date.now();
    const logger = loggerService.getLogger('mcp-tool', {
      tool: 'getProjectAvatarMetadata',
      serverType: ['datacenter'],
      operation: 'get_project_avatar_metadata',
      category: 'unknown',
    });

    try {
      logger.info('Getting project avatar metadata', {
        params: {
      ...
      const validatedParams = GetProjectAvatarMetadataSchema.parse(params);
      ...
      if (result.success) {
        ...
        logger.info('Project avatar metadata retrieved successfully', {
        ...
        logger.warn('Failed to get project avatar metadata', {
      ...
      logger.error('Get project avatar metadata tool error', {

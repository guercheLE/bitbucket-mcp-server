 ...
 */
const resolvePath = (path: string, parameters: Record<string, unknown>): { path: string; query: Record<string, unknown> } => {
...
export const createCallIdTool = (
    ...
    const logger = dependencies.logger ?? createLogger({ level: "info", defaultMeta: { scope: "tool:call-id" } });
    ...
    const handler = async (params: CallIdParamsOutput): Promise<unknown> => {
        ...
        try {
            ...
            const status = isRecord(error) && isRecord(error.response) && typeof error.response.status === "number"
                ? error.response.status
                : 500;
            const message = isRecord(error) && isRecord(error.response)

...
class DefaultMcpService implements McpService {
  ...
  private normalizeDependencies(dependencies: McpServiceDependencies): NormalizedDependencies {
    return {
      createClient: dependencies.createClient ?? ((info, options) => new Client(info, options)),
  ...
  private resolveEndpoint(endpoint: string | undefined, transport: TransportKind): URL {
    ...
    try {
      ...
      const invalidUrlError = new Error(`Invalid MCP endpoint URL: ${value}`);
      (invalidUrlError as Error & { cause?: unknown }).cause = error;
  ...
  private async createConnection(): Promise<void> {
    ...
    const client = this.dependencies.createClient(
      this.dependencies.clientInfo,
      this.dependencies.clientOptions,
    );

    client.onclose = () => this.handleClientClosed();
    client.onerror = (error: Error) =>
      this.dependencies.logger.error?.(`MCP transport error: ${error.message}`);

...
const loadEmbedder = async (): Promise<Embedder> => {
  ...
  const embed = (await module.pipeline(
    'feature-extraction',
    'sentence-transformers/all-MiniLM-L6-v2',
  )) as (text: string, options?: Record<string, unknown>) => Promise<unknown>;
...
export class VectorDBService {
  ...
  constructor(options: VectorDBServiceOptions = {}) {
    ...
    this.logger =
      options.logger ??
      createLogger({ level: 'info', defaultMeta: { scope: 'vector-db-service' } });
    this.databaseFactory =
      options.databaseFactory ?? ((path: string) => new Database(path, { readonly: true }));
  ...
  async search(query: string, options: VectorSearchOptions = {}): Promise<VectorSearchResult[]> {
    ...
    const rows = this.getSearchStatement(database).all(
  ...
  private getSearchStatement(database: BetterSqlite3Database): Statement<[string, number]> {
    if (!this.searchStatement) {
      this.searchStatement = database.prepare<[string, number], VectorRow>(

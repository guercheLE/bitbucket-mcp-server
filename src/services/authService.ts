...
export class AuthService {
  ...
  constructor(options: AuthServiceOptions = {}) {
    ...
    this.config = AuthConfigSchema.parse({
      priority: options.config?.priority ?? DEFAULT_PRIORITY,
    });
    this.strategies = new Map<AuthMethod, AuthStrategy>();

    if (options.strategies) {
      for (const [_method, strategy] of Object.entries(options.strategies) as [
        AuthMethod,
        AuthStrategy,
      ][]) {
        if (strategy) {
  ...
  async authenticateWithFallback(
    ...
    preferredOrder?: AuthMethod[],
  ): Promise<AuthFallbackResult> {
    const order =
      preferredOrder && preferredOrder.length > 0 ? preferredOrder : this.config.priority;
    ...
    for (const method of order) {
      ...
      if (typeof strategy.canHandle === 'function' && !strategy.canHandle(credentials)) {
        this.logger.debug('Skipping authentication strategy', {
          method,
          reason: 'canHandle returned false',
        });
        continue;
      }
      ...
      try {
        ...
        this.logger.warn('Authentication strategy failed', {
          method,

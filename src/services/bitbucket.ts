...
export type BitbucketServiceEvents = {
...
export class BitbucketService extends EventEmitter {
    ...
    override on(event: "connected", listener: (info: BitbucketServerInfo) => void): this;
    override on(event: "reconnected", listener: (info: BitbucketServerInfo) => void): this;
    override on(event: "disconnected", listener: () => void): this;
    override on(event: "error", listener: (error: BitbucketServiceError) => void): this;
    override on(event: string | symbol, listener: (...args: any[]) => void): this {
        return super.on(event, listener);
    }

    override once(event: "connected", listener: (info: BitbucketServerInfo) => void): this;
    override once(event: "reconnected", listener: (info: BitbucketServerInfo) => void): this;
    override once(event: "disconnected", listener: () => void): this;
    override once(event: "error", listener: (error: BitbucketServiceError) => void): this;
    ...
    private parseServerInfo(payload: any): BitbucketServerInfo {
        const version = String(payload?.version ?? payload?.buildNumber ?? payload?.buildVersion ?? "unknown");
        const type = this.resolveServerType(payload?.deploymentType ?? payload?.serverType);
    ...
    private normalizeError(error: unknown): BitbucketServiceError {
        ...
        const status = typeof (error as any)?.response?.status === "number" ? (error as any).response.status as number : undefined;

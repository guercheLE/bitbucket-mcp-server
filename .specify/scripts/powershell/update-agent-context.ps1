...
$cursorFile = Join-Path $repoRoot 'CURSOR.md'
...
switch ($AgentType) {
    'cursor' { Update-AgentFile $cursorFile 'Cursor Code' }
    ...
    '' {
        foreach ($pair in @(
            @{file=$cursorFile; name='Cursor Code'},
        ...
        if (-not (Test-Path $cursorFile) -and -not (Test-Path $geminiFile) -and -not (Test-Path $copilotFile) -and -not (Test-Path $cursorFile)) {
            Write-Output 'No agent context files found. Creating Cursor Code context file by default.'
            Update-AgentFile $cursorFile 'Cursor Code'
        }
    }
    Default { Write-Error "ERROR: Unknown agent type '$AgentType'. Use: cursor, gemini, copilot, cursor or leave empty for all."; exit 1 }
}
...
Write-Output ''
Write-Output 'Usage: ./update-agent-context.ps1 [cursor|gemini|copilot|cursor]'

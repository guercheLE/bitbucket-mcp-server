        ...
        if [ -f "$CONSTITUTION_FILE" ]; then
            ...
            if grep -q -i "test.*first\|tdd" "$CONSTITUTION_FILE"; then
                if [ -n "$FEATURE_DIR" ] && [ -f "$FEATURE_DIR/tasks.md" ]; then
                    ...
                    TEST_PHASE_LINE=$(grep -n "Phase.*Test\|Tests First" "$FEATURE_DIR/tasks.md" | head -1 | cut -d: -f1)
                    IMPL_PHASE_LINE=$(grep -n "Phase.*Core\|Phase.*Implementation" "$FEATURE_DIR/tasks.md" | head -1 | cut -d: -f1)
...
if [ -f "$CONSTITUTION_FILE" ]; then
    ...
    if grep -q "VERSION" "$CONSTITUTION_FILE"; then
        CONSTITUTION_VERSION=$(grep "VERSION" "$CONSTITUTION_FILE" | head -1 | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+' || echo "unknown")
